...
async def start_backup(filename: str, executor: ThreadPoolExecutor) -> None:
    try:
        path = Path(filename)
        with open(path) as csvfile:
            ...
            for index, row in tqdm(
                ...
                if len(row) < 2:
                    ...
                    continue
                await copy_changed_files_async(
                    Path(row[0]), Path(row[1]), executor)
...
if __name__ == '__main__':
    while True:
        try:
            ...
            asyncio.run(start_backup(FILENAME, executor))
